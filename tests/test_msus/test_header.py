...
def test_format_headers_synthetic():
    ...
    for i, buffer in enumerate(buffers):
        ...
        # but also since some pixels will *actually* be 682,
        # we allow diffs to also be 2 for the skips
...
def test_format_headers_raw(msus_raw_buffers):
    ...
    # Is the dropped buffer 0?
    # look at headers (from_header) and to_frame methods!
    # possibly look at the corner pixels (0, 255, 0, 255)
    # shorthand way of accessing code: error is in this part of the code, lets replicate it!
    # Maybe parsing headers is wrong, and we
    # can parse the header to find out why its not working.
    # dont need to display the images in the test pythons,
...
def test_buffer_to_array():
    """
    Checking to see if a 12x4 (4 12 bit pixels)
    converts to a known value in our buffer_2_array fxn
    """

    byte_sequence = bytes([0xC0, 0x1C, 0x01, 0xC0, 0x1C, 0x01])
    # byte_sequence = bytes([0x80, 0x08, 0x00, 0x80, 0x08, 0x00])
    # assert np.array_equal(sequence_16bit, np.array([0, 0, 0, 0]))
